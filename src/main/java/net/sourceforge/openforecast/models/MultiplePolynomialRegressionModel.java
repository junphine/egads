...
/**
 * Implements a multiple Polynomial variable regression model using the variables
...
public class MultiplePolynomialRegressionModel extends MultipleLinearRegressionModel
{
     ...
     */
    public MultiplePolynomialRegressionModel(int order) 
    {
       	super();
        this.order = order;
    }
     ...
     * @see #init(double,Hashtable)
     */
    public MultiplePolynomialRegressionModel(int order, String[] independentVariable)
    {
    ...
    public void init( DataSet dataSet )
    {
        ...
        if(this.newIndependentVariable==null){
        	newIndependentVariable = new String[n];
        	Arrays.fill(newIndependentVariable,"");
        	int degree = this.order;        
            int index = 0;
            int[] setTo = new int[this.independentVariable.length];            
    ...
    public double forecast( DataPoint dataPoint )
    {
        ...
        DataPoint dataPoint2 = this.transform(dataPoint);       
        
        return super.forecast(dataPoint2);
    }
    ...
    {
        return "Multiple Polynomial variable regression";
    }
    ...
    {
        String desc = "Multiple Polynomial variable regression model with the following equation:\n"
            +"  y="+intercept;
    ...
    {	
        ...
        do
        {
            for(int i = 0; i < setTo.length; i++)
                if(setTo[i] > 0)
                    newVec[index] *= Math.pow(x[i], setTo[i]);
            index++;
            curCount = increment(setTo, degree, curCount);
        }
        while(setTo[x.length-1] <= degree);
        
        Observation result = new Observation(dp.getDependentValue());
        for(int i=0;i<newVec.length;i++){
        	result.setIndependentValue(newIndependentVariable[i], newVec[i]);
        }
        return result;
    }
